...
window.onload=function(){
	...
	tusername.onblur=function(){
		...
		$.ajax({
		type:'post',
		url:'http://localhost:8080/zhaiShop/UserServlet?type=checkUserName', //发送数据的地址
		 data:{
			name:username
		 },
		 dataType: "json",
	...
	yzm.onblur=function(){
		...
		$.ajax({
			type:'post',
			url:'http://localhost:8080/zhaiShop/UserServlet?type=checkYZM', //发送数据的地址
			 data:{jyzm:yz},
			 dataType: "json",
			success:function(result){
				if(result.yzm=="false"){
